...
const savaStore = store => {
    store.subscribe((mutation, state) => {
        ...
        if (
            mutation.type == login.LOGIN_SUCCESS ||
            mutation.type == motion.NOTICE_UPDATE
        ) {
            //保存最新的state
            util.setStorage({
            ...
            if (mutation.type == login.LOGIN_SUCCESS) {
                ...
                console.log(state.login.AdminID);
                sessionStore.setItem(constants.USER_TOKEN, state.login.UserToken);
                Vue.http.headers.common["UserToken"] = String(state.login.UserToken); //请求头部文件
            }
